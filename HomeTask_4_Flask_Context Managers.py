...
import db_processing_Context_Managers
...
def list_vacancies():
    with db_processing_Context_Managers.DataBase() as db:
...
def vacancy_id(id_vac):
    with db_processing_Context_Managers.DataBase() as db:
        if request.method == 'GET':
            res_vacancy = db.query(f"SELECT * from vacancy where id = {id_vac}")
...
def vacancy_event(id_vac):
    with db_processing_Context_Managers.DataBase() as db:
        ...
        res_event = db.query(f"SELECT * from events where vacancy_id = {id_vac}")
...
def vacancy_event_id(id_vac, id_event):
    with db_processing_Context_Managers.DataBase() as db:
        if request.method == 'GET':
            res_id_event = db.query(f"SELECT * from events where vacancy_id = {id_vac} and event_id={id_event}")
